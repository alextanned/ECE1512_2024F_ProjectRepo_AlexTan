...
def get_dataset(dataset, data_root, transform=None, partition='train', syn=False, padd_syn=False):
...
def main():
    ...
    parser.add_argument('--padd_syn', action='store_true',default=False,help='use synthesized image from PADD')
    ...
    if args.syn:
        ...
        train_dataset = get_dataset(args.dataset, args.data_path, partition='train', padd_syn=True)
    else:
    ...
    if args.dataset == 'MHIST':
        model = get_network(args.model, channel=3, num_classes=2, im_size=(224, 224)).to(args.device)
    elif args.dataset == 'MNIST':
        model = get_network(args.model, channel=1, num_classes=10, im_size=(28, 28)).to(args.device)
