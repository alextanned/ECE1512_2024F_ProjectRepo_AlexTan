...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False):
    ...
    ##using SAM for evaluate synthetic data
    from sam import SAM
    base_optim = torch.optim.SGD
    optimizer = SAM(net.parameters(), base_optim,lr=lr, momentum=args.mom, weight_decay=args.l2,rho=args.rho)
    ####
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        if ep in lr_schedule:
            lr *= 0.1
            #optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
            optimizer.base_optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=args.mom, weight_decay=args.l2)
